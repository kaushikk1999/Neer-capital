...
// All three languages of the AI-translated text are embedded up front, so
// switching language is a pure client-side swap with no server round-trip.
type LocaleMaps = Record<Locale, Record<string, string>>

export interface ReportViewProps {
  slug: string
  dateLabel: string
  hasSummary: boolean
  metrics: { id: string; value: string; unit: string | null; period: string | null }[]
  charts: { id: string; type?: string; config?: unknown; configV2?: unknown }[]
  sections: { id: string; hasExcerpt: boolean }[]
...
export function ReportView(props: ReportViewProps) {
  ...
  const map = props.loc[locale] ?? props.loc.en
  // Text comes from the selected language, falling back to English, then empty.
  const tr = (key: string) => map[key] ?? props.loc.en[key] ?? ''
    ...
    <div className="min-h-screen bg-[#050505] text-gray-100 font-sans selection:bg-blue-500/30">
      <div className="max-w-6xl mx-auto px-4 py-12 md:py-24">
        ...
        <header className="mb-16 relative flex flex-col md:flex-row md:items-start justify-between gap-6">
          ...
          <a
            href={`/api/documents/${props.slug}/file`}
            target="_blank"
        ...
        <div className="mb-16">
          <div className="space-y-4 p-8 rounded-3xl bg-white/[0.02] border border-white/[0.05] backdrop-blur-xl relative overflow-hidden group">
            ...
            <p className="text-lg md:text-xl leading-relaxed text-gray-200 whitespace-pre-wrap">
              {props.hasSummary ? tr('summary') : t('report.noSummary')}
            </p>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {props.metrics.map((metric) => (
                <div key={metric.id} className="p-6 rounded-2xl bg-gradient-to-b from-white/[0.04] to-transparent border border-white/[0.05] hover:border-white/[0.1] transition-colors relative overflow-hidden">
                  ...
                  <p className="text-sm text-gray-400 mb-2 truncate" title={tr(`metric.${metric.id}.label`)}>{tr(`metric.${metric.id}.label`)}</p>
          ...
          <section className="mb-20">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {props.charts.map((chart) => (
                <ReportChart key={chart.id} chart={{ ...chart, title: tr(`chart.${chart.id}.title`) }} />
              ))}
            </div>
          </section>
        )}
              ...
              {props.sections.map((section) => (
                <div key={section.id} className="group flex flex-col md:flex-row gap-8 items-start p-8 rounded-3xl bg-white/[0.01] hover:bg-white/[0.03] border border-transparent hover:border-white/[0.05] transition-all duration-300">
                  <div className="md:w-1/3 shrink-0">
                    <h4 className="text-xl font-semibold text-white group-hover:text-blue-400 transition-colors">{tr(`section.${section.id}.heading`)}</h4>
                  </div>
                  <div className="md:w-2/3 space-y-4">
                    <p className="text-gray-300 leading-relaxed whitespace-pre-wrap">{tr(`section.${section.id}.content`)}</p>
                    {section.hasExcerpt && (
                      <blockquote className="pl-4 border-l-2 border-blue-500/30 text-sm text-gray-500 italic">
                        &quot;{tr(`section.${section.id}.excerpt`)}&quot;
                      </blockquote>
                    )}
                  </div>
                </div>
                ...
                <ul className="space-y-6 text-gray-300 list-none pl-0">
                  {props.risks.map((risk, i) => (
                    <li key={i} className="pl-4 border-l border-red-500/30">
                      <strong className="text-white block mb-1 text-lg">{tr(`risk.${i}.risk`)}</strong>
                      <span className="text-gray-400 block mb-3 leading-relaxed">{tr(`risk.${i}.explanation`)}</span>
                      {risk.hasEvidence && (
                        <em className="text-xs text-red-400/70 bg-red-950/50 px-2 py-1 rounded inline-block">{t('report.source')} &quot;{tr(`risk.${i}.evidence`)}&quot;</em>
                      )}
                    </li>
                  ))}
                </ul>
        ...
        <FeedbackPrompt reportSlug={props.slug} reportTitle={props.loc.en['title'] ?? ''} />
