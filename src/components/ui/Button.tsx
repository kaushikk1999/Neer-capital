...
type ButtonProps = { children: ReactNode; href?: string; onClick?: () => void; variant?: 'primary' | 'secondary' | 'ghost'; size?: 'sm' | 'md' | 'lg'; icon?: ReactNode; fullWidth?: boolean; className?: string };
...
export function Button({ children, href, onClick, variant = 'primary', size = 'md', icon, fullWidth, className }: ButtonProps) { const classes = cn(base, variants[variant], sizes[size], fullWidth && 'w-full', className); if (href) { return <Link href={href} className={classes}><span>{children}</span>{icon}</Link>; } return <button className={classes} onClick={onClick}><span>{children}</span>{icon}</button>; }
